...
for i in range(1, 20):
    url = 'http://washingtonmonthly.com/college_guide?ranking=best-bang-for-the-buck-northeast?page=' + str(i)
    driver.get(url)
    ...
    try:
        for row in driver.find_elements_by_xpath("//*[starts-with(@id,'table')]/tbody/tr"):
            ...
            data.insert(len(data), 'Washington Monthly - Best Bang for the Buck - Northeast')
...
print(df)
df.to_csv('C:\\Users\\Saurabh Pore\\Desktop\\NJIT\\RA\\Final\\WM-Best Bang for the Buck - Northeast.csv', index=False)
print(df)
driver.quit()
